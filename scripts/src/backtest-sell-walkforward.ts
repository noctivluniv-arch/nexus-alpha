/**
 * backtest-sell-walkforward.ts
 *
 * Walk-forward validation untuk SELL yang SUDAH LIVE di production sekarang:
 *   confidence 45-55, bias BEARISH, trend1d juga BEARISH (searah)
 *
 * Data dibagi 2 secara KRONOLOGIS (bukan acak):
 *   - Periode 1 (60% pertama, lebih lama)
 *   - Periode 2 (40% terakhir, lebih baru)
 *
 * Kalau edge asli, KEDUA periode harusnya sama-sama positif (WR>=50%, AvgPnL>0).
 *
 * Jalankan:
 *   npx tsx scripts/src/backtest-sell-walkforward.ts
 */
...
interface Result { pair: string; pnlPct: number; win: boolean }
...
function simulateTrade(daily: Candles, entryIdx: number, atr14: number): { pnlPct: number; win: boolean } {
  const entry = daily.closes[entryIdx];
  const riskAmt = atr14 * 1.5;
  const sl = entry + riskAmt;
  const tp = entry - riskAmt * 1.5;
...
function runOnRange(pair: string, daily: Candles, h4: Candles, startIdx: number, endIdx: number): Result[] {
  ...
  for (let i = startIdx; i < endIdx; i++) {
    ...
    // PERSIS logika production sekarang: SELL, bias BEARISH, conf 45-55, trend1d juga BEARISH
    if (bias === "BEARISH" && conf >= 45 && conf <= 55 && inp.trend1d === "BEARISH") {
      const sim = simulateTrade(daily, i, inp.atr14);
      results.push({ pair, pnlPct: sim.pnlPct, win: sim.win });
    }
  }
  return results;
}
...
function printSummary(title: string, results: Result[]) {
  ...
  console.log(`  Trades: ${results.length} | WinRate: ${wr.toFixed(1)}% | AvgPnL: ${avgPnl.toFixed(2)}% | PF: ${pf.toFixed(2)}${mark}`);
}

async function main() {
  console.log("=".repeat(72));
  console.log("WALK-FORWARD VALIDATION — SELL (production: conf 45-55, BEARISH, trend1d searah)");
  console.log("=".repeat(72));

  let period1All: Result[] = [];
  let period2All: Result[] = [];
  let p1Start = "", p1End = "", p2End = "";
  ...
  for (const pair of PAIRS) {
    ...
    if (pair === PAIRS[0]) {
      p1Start = new Date(daily.times[MIN_HISTORY]).toISOString().slice(0, 10);
      p1End = new Date(daily.times[splitIdx]).toISOString().slice(0, 10);
      p2End = new Date(daily.times[daily.closes.length - 1 - TRADE_MAX_BARS - 1]).toISOString().slice(0, 10);
    }

    period1All.push(...runOnRange(pair, daily, h4, MIN_HISTORY, splitIdx));
    period2All.push(...runOnRange(pair, daily, h4, splitIdx, daily.closes.length - TRADE_MAX_BARS - 1));
  }
  ...
  console.log(`\n${"=".repeat(72)}`);
  console.log("KESIMPULAN: kalau KEDUA periode ✅, edge SELL yang live sekarang kemungkinan ASLI.");
  console.log("Kalau cuma 1 periode ✅, kemungkinan besar OVERFITTING pada backtest sebelumnya.");
  console.log("=".repeat(72));
}
