/**
 * retrain-with-volatility.ts
 *
 * Tambah fitur rolling_vol_pct (percentile volatilitas rolling 90 hari per
 * pair, TANPA lookahead) ke 37 fitur lama, retrain BUY & SELL, lalu ulangi
 * walk-forward 4-fold yang sama seperti sebelumnya untuk lihat apakah fold
 * yang gagal (BUY fold 1&3) jadi stabil, dan pastikan SELL tidak memburuk.
 *
 * Dataset sudah diperbesar ke ~9200 baris (dari 2020, sebelumnya cuma ~2.3 tahun).
 * Walk-forward sekarang 8 fold (naik dari 4) untuk uji lebih banyak regime pasar.
 *
 * Jalankan:
 *   tsx scripts/src/retrain-with-volatility.ts
 */
...
function rollingPercentile(values: number[], window: number): number[] {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    if (windowVals.length < 10) { result.push(0.5); continue; } // netral saat warm-up, bukan buang baris
...
function walkForward(rows: Row[], featureNames: string[], side: "buy" | "sell") {
  ...
  console.log(`\n${"=".repeat(90)}`);
  console.log(`WALK-FORWARD (37 fitur lama + rolling_vol_pct = ${featureNames.length} fitur) — ${side.toUpperCase()}`);
  ...
  console.log(`\n${side.toUpperCase()} — ${successCount}/${foldCount} fold berhasil (sebelumnya ${side === "buy" ? "2/4" : "3/4"} tanpa fitur volatilitas)`);
}
...
async function main() {
  ...
  console.log(`Fitur bertambah dari ${baseFeatures.length} -> ${featureNames.length} (tambah: rolling_vol_pct)`);

  walkForward(enrichedRows, featureNames, "buy");
  walkForward(enrichedRows, featureNames, "sell");
}
