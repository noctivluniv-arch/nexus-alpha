...
// Deteksi token baru yang sedang aktif marketing (Boosted + Token Profile)
// SEBELUM masuk ke GeckoTerminal/meme scanner biasa — lebih early, lebih spekulatif

const dexRadarCooldown = new Map<string, number>(); // address → last alert ts
...
async function runDexRadarScan() {
  ...
  try {
    ...
    // Buat index dari profiles berdasarkan address lowercase
    const profileMap = new Map<string, DexToken>();
    for (const p of profiles) {
      profileMap.set(p.tokenAddress.toLowerCase(), p);
    }

    // Cari crossmatch: token yang ada di Boosted DAN Profiles
    const now = Date.now();
    let sent = 0;
    ...
    for (const token of boosted) {
      ...
      if (!profile) continue; // tidak ada di profiles, skip

      // Cooldown check
      const lastSent = dexRadarCooldown.get(addr) ?? 0;
      if (now - lastSent < DEX_RADAR_COOLDOWN_MS) continue;

      // Skip chain yang tidak relevan (terlalu obscure)
      ...
      // Harus punya minimal Twitter atau website supaya ada tanda marketing nyata
...
`;
      if (isCto) msg += `  • CTO (Community Takeover) — komunitas ambil alih
`;
      msg += `
`;
